...
# Hotspot version
HOTSPOT_VERSION="V0.913"
opt1="$1"
opt2="$2"
opt3="$3"
opt4="$4"

# sname: Name of the hotspot
snam="R1P47i04"

# eth_dev: Ethernet port
eth_dev="eth0"
...
function Hotspot_Start {
  Hotspot_Defines
  logger -s -t $snam "starting  hotspot $HOTSPOT_VERSION"
  ...
  if [ "$wle_avail" == "yes" ] || [ "$eth_ip" != "" ] || [ "yes" == "yes" ]; then
    ...
    msg="select same channel for ap & station mode (currently not supported)"
    ...
    setchan)
     ...
     syslog)
        ...
        tor)
            ...
            ;;
    version)
            echo "$HOTSPOT_VERSION"
            ;;
     status)
            Hotspot_Status
            ;;
          *)
  echo "Usage: $snam try | start [nowlan] | stop [nowlan] | retry | restart [nowlan] | status | setup | setchan [channel] | syslog [lines] | tor [start|stop] | modpar <crda|dnsmasq|hostapd> <name> [value] | version | wlan start|stop [$wli_dev|$wle_dev]"
  exit 1
esac
